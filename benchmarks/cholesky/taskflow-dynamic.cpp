...
void choleskyDynamicTaskflow(std::vector<std::vector<double>>& matrix, 
  ...
  for (size_t j = 0; j < n; ++j) {
    // Diagonal task
    auto diag_task = executor.dependent_async([&matrix, j]() {
      matrix[j][j] = sqrt(matrix[j][j]);
    });

    // Column tasks
    std::vector<tf::Future<void>> col_tasks;
    for (size_t i = j + 1; i < n; ++i) {
      col_tasks.push_back(
        executor.dependent_async([&matrix, j, i]() {
    ...
    for (size_t i = j + 1; i < n; ++i) {
      for (size_t k = j + 1; k <= i; ++k) {
        executor.dependent_async([&matrix, j, i, k]() {
